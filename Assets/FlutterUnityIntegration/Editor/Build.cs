...
public class Build : EditorWindow
{
    ...
    static readonly string androidExportPath =
    ...
    static readonly string iosExportPluginPath =
    ...
    public static void DoBuildAndroid(String buildPath, bool isPlugin)
    {
        ...
        //This part exists for compiling application in release mode.
        build_text += @"
android {
    buildTypes {
        release {
    ...
    [MenuItem("Flutter/RefreshXCode", false, 6)]
    public static void DoRefreshXCode()
    {
        File.Copy("Assets/FlutterUnityIntegration/Editor/XCodePostBuild.cs", "../packages/XCodePostBuild.cs");
        AssetDatabase.DeleteAsset("Assets/FlutterUnityIntegration/Editor/XCodePostBuild.cs");

        File.Move("../packages/XCodePostBuild.cs", "Assets/FlutterUnityIntegration/Editor/XCodePostBuild.cs");
        AssetDatabase.ImportAsset("Assets/FlutterUnityIntegration/Editor/XCodePostBuild.cs");
    }
...
";
    ...
    static void SetupAndroidNdk(string androidPath)
    {
        var localPropertiesPath = Path.Combine(androidPath, "local.properties");

        // Modify local.properties
        var localPropertiesText = File.ReadAllText(localPropertiesPath);
        var localPropertiesLines = localPropertiesText.Split(new[] {Environment.NewLine},
            StringSplitOptions.None).ToList();
        for (int i = 0; i < localPropertiesLines.Count; i++)
        {
            var line = localPropertiesLines[i];
            if (line.Contains("ndk.dir"))
            {
                localPropertiesLines.RemoveAt(i);
            }
        }

        localPropertiesLines.Add($"ndk.dir={AndroidExternalToolsSettings.ndkRootPath}");
        localPropertiesText = string.Join("\n", localPropertiesLines);
        Console.Write($"Placed ndk.dir{AndroidExternalToolsSettings.ndkRootPath} inside local properties");
        File.WriteAllText(localPropertiesPath, localPropertiesText);
    }
    ...
    static void SetupIOSProjectForPlugin()
    {
        ...
        if (!Regex.IsMatch(pubsec_text, @"\w\.xcconfig(?:[^}]*})+") &&
    ...
    static async void BuildUnityFrameworkArchive()
    {
        ...
        try
        {
            ...
            await
                $"archive -workspace {XCPROJECT_NAME} -scheme {SCHEME_NAME} -sdk iphoneos -archivePath {BUILD_PATH}/Release-iphoneos.xcarchive ENABLE_BITCODE=NO |xcpretty"
                    .Bash("xcodebuild");

            Debug.Log("### Copying framework files");
            await $" -RL {IOS_ARCHIVE_FRAMEWORK_PATH} {IOS_RUNNER_PATH}/{FRAMEWORK_NAME_WITH_EXT}".Bash("cp");
            await $" -RL {IOS_ARCHIVE_FRAMEWORK_PATH}/{DYSM_NAME_WITH_EXT} {IOS_RUNNER_PATH}/{DYSM_NAME_WITH_EXT}"
                .Bash("cp");
